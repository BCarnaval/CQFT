...
int main(int argc, const char *argv[]) {
  ...
  const char *condHeaders[] = {"#mu",      "density",  "sigma_xx", "sigma_xy",
                               "alpha_xx", "alpha_xy", "beta_xx",  "beta_xy"};

  const char *sWeightHeaders[] = {"#kx", "ky", "A(omega, E)"};
  ...
  double energyCutoff =
      2. * 2. * acosh(0.25 * sqrt(beta / amplitudeCutoff)) / beta;
  ...
  for (int m = 0; m < nMu; m++) {
    ...
    for (int i = 0; i < nK; i++) {
      ...
      for (int j = 0; j < nK; j++) {
        ...
        double epsilon_k = -2. * t * (cosk[i] + cosk[j]) -
                           4. * tp * cosk[i] * cosk[j] -
                           2. * tpp * (cos2k[i] + cos2k[j]) - mu;
        double depsilon_k_dkx = 2. * t * sink[i] + 4. * tp * sink[i] * cosk[j] +
                                4. * tpp * sin2k[i];
        double depsilon_k_dky = 2. * t * sink[j] + 4. * tp * cosk[i] * sink[j] +
                                4. * tpp * sin2k[j];
        double ddepsilon_k_dkx_dkx = 2. * t * cosk[i] +
                                     4. * tp * cosk[i] * cosk[j] +
                                     8. * tpp * cos2k[i];
        double ddepsilon_k_dky_dky = 2. * t * cosk[j] +
                                     4. * tp * cosk[i] * cosk[j] +
                                     8. * tpp * cos2k[j];
        ...
        double kernel_xy =
            -(1. / 3.) * (dE1_k_dkx * dE1_k_dkx * ddE1_k_dky_dky +
                          dE1_k_dky * dE1_k_dky * ddE1_k_dkx_dkx -
                          2.0 * dE1_k_dkx * dE1_k_dky * ddE1_k_dkx_dky);
        // double kernel_xy =
        //     -(2. / 3.) * (dE1_k_dkx * (dE1_k_dkx * ddE1_k_dky_dky -
        //                                dE1_k_dky * ddE1_k_dkx_dky));
        ...
        for (int n = 0; n < nOmega; n++) {
          ...
          double frequencyKernel_xx =
              -dfermiDirac_dw[n] * kernel_xx * A1_k * A1_k;
          double frequencyKernel_xy =
              -dfermiDirac_dw[n] * kernel_xy * A1_k * A1_k * A1_k;
    ...
    fprintf(fileOut, "% 4.8e % 4.8e  ", f0 * sigma_xx, sigma_xy);
    fprintf(fileOut, "% 4.8e % 4.8e  ", f0 * alpha_xx, f0 * alpha_xy);
    fprintf(fileOut, "% 4.8e % 4.8e", f0 * beta_xx, f0 * beta_xy);
    fprintf(fileOut, "\n");
  }
